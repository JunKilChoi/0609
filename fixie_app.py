...
st.markdown("## 속도가 조금 빨라졌을 뿐인데, 왜 정지거리는 훨씬 길어질까?")

st.markdown(
    r"""
자전거를 타고 달리다가 앞에 사람이 나타났다고 생각해 보자. 우리는 보통 브레이크를 잡으면 멈춘다고 생각하지만, 실제로는 위험을 본 순간 자전거가 바로 멈추지 않는다. 먼저 눈으로 위험을 발견하고, 뇌가 상황을 판단하고, 손이나 발이 제동 동작을 시작한다. 그다음에야 타이어와 노면 사이의 마찰력이 작용하여 자전거의 속력이 줄어든다.

따라서 자전거가 완전히 멈출 때까지의 거리는 두 부분으로 나눌 수 있다.
"""
)
st.latex(r"\text{총 정지거리}=\text{반응거리}+\text{제동거리}")

st.markdown(
    r"""
반응거리는 위험을 발견했지만 아직 제동하지 못한 채 이동한 거리이고, 제동거리는 제동을 시작한 뒤 완전히 멈출 때까지 이동한 거리이다. 이 탐구의 핵심은 이 두 거리가 속도에 따라 어떻게 달라지는지 확인하는 것이다.

특히 중요한 점은 제동거리가 속도에 단순히 비례하지 않는다는 것이다. 속도가 커지면 제동거리는 속도의 제곱에 가깝게 증가한다. 그래서 정지거리 문제는 수학적으로 이차함수와 연결된다.

**핵심 정리**  
위험을 본 순간 바로 멈추는 것이 아니라, 반응거리와 제동거리가 합쳐져 총 정지거리가 만들어진다.
"""
)

st.markdown("### 3-1. 반응거리: 위험을 보고도 아직 제동하지 못하는 거리")
st.markdown(
    r"""
위험을 발견한 뒤 실제로 제동하기까지는 시간이 걸린다. 이 시간을 반응 시간이라고 한다. 반응 시간 동안 자전거는 아직 제동하지 않았으므로 거의 원래 속력으로 계속 움직인다.

따라서 반응거리는 다음과 같이 나타낼 수 있다.
"""
)
...
st.markdown(
    r"""
여기서 $v$는 자전거의 초기 속력, $t_r$은 반응 시간이다. 이 식은 중학교에서 배우는 기본 속력 공식과 같다.
"""
)
st.latex(r"\text{거리}=\text{속력}\times\text{시간}")

st.markdown(
    r"""
반응 시간이 일정하다면, 속력이 2배가 될 때 반응거리도 2배가 된다. 즉 반응거리는 속력에 정비례한다. 그래프 관점에서 보면 반응거리는 일차함수적인 증가를 보인다.

예를 들어 반응 시간이 1초라면, 자전거는 위험을 본 뒤에도 1초 동안 그대로 앞으로 나아간다. 10 km/h일 때보다 30 km/h일 때 1초 동안 훨씬 더 멀리 이동하므로, 위험을 빨리 발견하더라도 속도가 빠르면 이미 상당한 거리를 지나가게 된다.

**핵심 정리**  
반응거리는 속력에 비례한다. 반응 시간이 길수록, 속력이 빠를수록 위험을 본 뒤에도 더 멀리 이동한다.
"""
)

st.markdown("### 3-2. 제동거리: 브레이크를 잡아도 바로 멈추지 않는 이유")
st.markdown(
    r"""
제동을 시작하면 자전거의 속력은 점점 줄어든다. 이때 자전거의 운동을 방해하는 힘이 작용하는데, 그 힘이 바로 마찰력이다.

자전거 타이어와 도로 사이의 마찰이 클수록 자전거는 더 잘 멈춘다. 마른 아스팔트에서는 비교적 잘 멈추지만, 젖은 길이나 낙엽길, 빙판길에서는 쉽게 미끄러진다. 이 차이를 수치로 나타낸 것이 마찰계수 $\mu$이다.

마찰계수가 클수록 노면이 타이어를 잘 붙잡아 주고, 마찰계수가 작을수록 미끄러지기 쉽다.

제동할 때 타이어와 노면 사이에 작용하는 마찰력은 단순화하면 다음과 같이 나타낼 수 있다.
"""
)
st.latex(r"F_{\text{마찰}}\approx \mu mg")

st.markdown(
    r"""
여기서 $m$은 자전거와 사람을 합친 질량, $g$는 중력가속도이다. 마찰력은 자전거가 움직이는 방향과 반대 방향으로 작용하여 속력을 줄인다.

하지만 마찰력이 작용한다고 해서 자전거가 즉시 멈추는 것은 아니다. 달리는 자전거는 이미 운동에너지를 가지고 있기 때문이다. 제동이란 그 운동에너지를 마찰을 통해 줄여 가는 과정이다.

**핵심 정리**  
제동거리는 타이어와 노면 사이의 마찰에 의해 결정된다. 노면이 미끄러울수록 마찰계수가 작아지고 제동거리가 길어진다.
"""
)

st.markdown("### 3-3. 운동에너지: 속도가 빠를수록 위험이 급격히 커지는 이유")
st.markdown(
    r"""
달리는 물체는 운동에너지를 가진다. 운동에너지는 질량이 클수록, 속력이 빠를수록 커진다.
"""
)
...
st.markdown(
    r"""
이 식에서 가장 중요한 부분은 $v^2$이다. 운동에너지는 속력에 비례하는 것이 아니라, 속력의 제곱에 비례한다.

즉 속력이 2배가 되면 운동에너지는 2배가 아니라 4배가 된다. 속력이 3배가 되면 운동에너지는 9배가 된다. 이것이 빠른 속도가 위험한 가장 중요한 이유이다.

자전거를 멈추려면 이 운동에너지를 줄여야 한다. 제동 과정에서 운동에너지는 사라지는 것이 아니라 열, 소리, 타이어와 노면의 변형 등 다른 형태의 에너지로 전환된다. 시뮬레이션에서 보이는 연기나 미끄럼 자국은 이 에너지 전환을 눈에 보이게 표현한 것이다.

**핵심 정리**  
운동에너지는 속력의 제곱에 비례한다. 따라서 속력이 조금만 증가해도 멈추기 위해 줄여야 할 에너지는 훨씬 크게 증가한다.
"""
)

st.markdown("### 3-4. 일과 에너지로 제동거리 구하기")
st.markdown(
    r"""
과학에서 힘이 물체를 이동시키거나 물체의 운동 상태를 바꿀 때, 힘이 일을 한다고 말한다. 일은 다음과 같이 나타낼 수 있다.
"""
)
st.latex(r"W=Fd")
...
st.markdown(
    r"""
제동할 때는 마찰력이 자전거의 운동 방향과 반대로 작용한다. 이 마찰력이 일을 하면서 자전거의 운동에너지가 줄어든다. 자전거가 완전히 멈추면 운동에너지는 0이 된다. 따라서 처음에 가지고 있던 운동에너지가 마찰에 의해 다른 형태의 에너지로 전환되었다고 볼 수 있다.

처음 운동에너지와 마찰력이 한 일을 연결하면 다음과 같다.
"""
)
st.latex(r"\frac{1}{2}mv^2=\mu mgd")

st.markdown(
    r"""
왼쪽은 자전거가 처음 가진 운동에너지이고, 오른쪽은 마찰력이 제동거리 $d$ 동안 한 일이다.
...
여기서 제동거리 $d$는 속력 $v$의 제곱에 비례한다. 그래서 속력이 커질수록 제동거리는 단순히 조금 늘어나는 것이 아니라 점점 더 빠르게 길어진다.

**핵심 정리**  
제동거리는 $\frac{v^2}{2\mu g}$로 나타낼 수 있다. 속력의 제곱이 들어가기 때문에 정지거리 그래프가 이차함수 형태를 가진다.
"""
)

st.markdown("### 3-5. 질량을 바꾸어도 정지거리가 직접 변하지 않는 이유")
st.markdown(
    r"""
처음에는 사람이 무거우면 더 늦게 멈추지 않을까 생각하기 쉽다. 실제 생활 감각으로는 자연스러운 생각이다. 질량이 큰 물체는 운동에너지도 크기 때문이다.
"""
)
st.latex(r"E_k=\frac{1}{2}mv^2")
...
st.markdown(
    r"""
하지만 질량이 커지면 노면과 타이어 사이의 마찰력도 함께 커진다.
"""
)
st.latex(r"F_{\text{마찰}}\approx \mu mg")
...
st.markdown(
    r"""
즉 질량이 커지면 멈추어야 할 운동에너지도 커지지만, 동시에 제동에 사용되는 마찰력도 커진다. 단순화한 이상적인 마찰 모델에서는 이 두 효과가 서로 상쇄된다.
"""
)
st.latex(r"\frac{1}{2}mv^2=\mu mgd")

st.markdown("양쪽에 모두 $m$이 있으므로 약분하면,")
st.latex(r"d=\frac{v^2}{2\mu g}")
...
그래서 이 앱의 기본 모델에서는 질량을 바꾸어도 정지거리 계산값이 직접 변하지 않는다. 물론 현실에서는 타이어 상태, 브레이크 성능, 무게 중심, 노면 상태 등이 함께 작용한다. 하지만 이 탐구에서는 중학교 수준의 단순 모델을 사용하여 질량이 커지면 운동에너지도 커지지만 마찰력도 함께 커진다는 관계를 확인한다.

**핵심 정리**  
단순 마찰 모델에서는 질량이 식에서 약분된다. 그래서 질량은 현실감 있는 변수이지만, 이 모델의 정지거리에는 직접 영향을 주지 않는다.
"""
)

st.markdown("### 3-6. 총 정지거리 식 만들기")
st.markdown(
    r"""
이제 반응거리와 제동거리를 합치면 총 정지거리 식을 만들 수 있다.
...
st.markdown("제동거리는 다음과 같다.")
st.latex(r"\text{제동거리}=\frac{v^2}{2\mu g}")

st.markdown("따라서 총 정지거리 $S(v)$는 다음과 같다.")
st.latex(r"S(v)=vt_r+\frac{v^2}{2\mu g}")

st.markdown(
    r"""
이 식에서 $vt_r$은 속력에 비례하는 항이다. 반응 시간이 일정하다면 속력이 커질수록 반응거리도 같은 비율로 커진다. 그래서 이 부분은 정지거리 식에서 일차항에 해당한다.

반면 $\frac{v^2}{2\mu g}$은 속력의 제곱에 비례하는 항이다. 속력이 2배가 되면 제동거리는 4배에 가까워지고, 속력이 3배가 되면 제동거리는 9배에 가까워진다. 그래서 이 부분은 정지거리 식에서 이차항에 해당한다.

앱의 그래프에서는 속력을 km/h 단위로 표시하므로, 정지거리 식의 계수는 다음과 같이 정리된다.
"""
)
st.latex(r"A=\frac{1}{25.92\mu g}")
st.latex(r"B=\frac{t_r}{3.6}")

st.markdown("따라서 총 정지거리 식은 다음과 같은 이차함수 형태로 볼 수 있다.")
st.latex(r"S(v)=Av^2+Bv+0")

st.markdown(
    r"""
여기서 $A$는 $v^2$ 앞의 계수로, 마찰계수 $\mu$와 중력가속도 $g$로 이루어진 값이다. $B$는 $v$ 앞의 계수로, 반응 시간 $t_r$로 이루어진 값이다. 즉 $A$는 제동거리 항과 관련되고, $B$는 반응거리 항과 관련된다.

정지거리 그래프를 볼 때는 $A$가 커지면 그래프가 더 가파르게 올라가고, $B$가 커지면 같은 속도에서 정지거리가 더 길어진다고 이해하면 된다.

**핵심 정리**  
총 정지거리는 반응거리와 제동거리의 합이다. 반응거리는 일차항 $Bv$, 제동거리는 이차항 $Av^2$이 되어 전체 식은 $S(v)=Av^2+Bv+0$ 꼴의 이차함수로 나타난다.
"""
)

st.markdown("### 3-7. 그래프를 어떻게 읽어야 할까?")
st.markdown(
    r"""
그래프에서 가로축은 초기 속력 $v$, 세로축은 총 정지거리 $S(v)$이다. 오른쪽으로 갈수록 속력이 커지고, 위로 갈수록 멈추는 데 필요한 거리가 길어진다.

이 그래프는 직선이 아니라 포물선 모양이다. 총 정지거리 안에 $v^2$이 들어 있기 때문이다. 낮은 속도에서는 그래프가 비교적 완만하게 보이지만, 속력이 커질수록 그래프는 점점 더 가파르게 올라간다.

이것은 현실적인 의미를 가진다. 낮은 속도에서 5 km/h 차이는 크게 느껴지지 않을 수 있다. 하지만 높은 속도에서는 같은 5 km/h 증가라도 정지거리를 훨씬 크게 늘릴 수 있다. 즉 빠르게 달릴수록 속도를 조금 더 높이는 행동은 더 큰 위험 증가로 이어진다.

이차함수에서 $A$는 그래프가 얼마나 가파르게 올라가는지를 결정한다. 이 앱에서는 마찰계수 $\mu$가 작아질수록 $A$가 커진다. 따라서 빙판길이나 낙엽길처럼 미끄러운 노면에서는 그래프가 더 가파르게 올라간다.

반응 시간 $t_r$이 커지면 $B$가 커진다. 같은 속력이라도 반응 시간이 길수록 위험을 보고도 더 멀리 이동하므로 총 정지거리가 길어진다.

**핵심 정리**  
정지거리 그래프는 포물선 형태이다. 노면이 미끄러울수록, 반응 시간이 길수록 같은 속도에서도 더 멀리 가서 멈춘다.
"""
)

st.markdown("### 3-8. 제동거리를 그래프로 한눈에 보는 방법: 완전제곱식으로 정리")
st.markdown(
    r"""
정지거리 식은 다음과 같은 이차함수 형태로 나타낼 수 있다.
"""
)
...
st.markdown(
    r"""
이 식만 보아도 정지거리가 속력에 대한 이차함수라는 것은 알 수 있다. 하지만 그래프의 모양을 더 잘 이해하려면 완전제곱식으로 정리해 볼 수 있다.

완전제곱식으로 바꾸는 과정은 다음과 같다.
"""
)
st.latex(r"S(v)=Av^2+Bv")
...
st.markdown("따라서 정지거리 함수는 다음과 같이 완전제곱식으로 정리된다.")
st.latex(r"S(v)=A\left(v+\frac{B}{2A}\right)^2-\frac{B^2}{4A}")

st.markdown("이 식을 보면 정지거리 그래프가 기본 포물선 $y=Av^2$을 평행이동한 형태임을 알 수 있다. 또한 꼭짓점은 다음과 같이 나타난다.")
st.latex(r"\left(-\frac{B}{2A},-\frac{B^2}{4A}\right)")

st.markdown(
    r"""
다만 이 꼭짓점은 보통 음수 속도 영역에 있다. 실제 자전거의 속력은 음수가 될 수 없으므로, 꼭짓점은 실제 주행 상황에서 직접 나타나는 지점은 아니다. 앱의 그래프에서 $v<0$인 부분을 점선으로 표시하는 이유도 이 때문이다. 점선 구간은 실제 자전거 운동이 아니라, 이차함수의 전체 모양을 이해하기 위한 수학적 확장이다.

완전제곱식으로 정리하면 그래프를 해석하기 쉬워진다. $A$가 양수이므로 그래프는 위로 열린 포물선이고, $A$가 커질수록 더 가파르게 올라간다. 따라서 마찰계수가 작은 노면에서는 속도가 조금만 커져도 정지거리가 빠르게 증가한다. 반응 시간이 길어지면 $B$가 커져 같은 속도에서도 총 정지거리가 더 길어진다.

**핵심 정리**  
완전제곱식은 정지거리 그래프의 전체 모양을 보기 위한 방법이다. 정지거리 함수는 위로 열린 포물선이며, 실제 의미가 있는 구간은 속력이 0 이상인 부분이다.
"""
)

st.markdown("### 3-9. 이 탐구가 보여주는 핵심")
st.markdown(
    r"""
이 탐구는 단순히 자전거가 몇 m 뒤에 멈추는지 계산하는 활동이 아니다. 현실의 안전 문제 안에 숨어 있는 수학적 구조를 찾는 활동이다.

과학적으로 보면 자전거의 제동은 운동에너지, 마찰력, 일, 에너지 전환의 문제이다. 수학적으로 보면 반응거리의 일차항과 제동거리의 이차항이 결합된 이차함수의 문제이다.
"""
)
st.latex(r"S(v)=Av^2+Bv+0")
...
st.markdown(
    r"""
이 식 하나에는 여러 의미가 들어 있다. 속도가 커지면 정지거리가 빠르게 증가한다. 반응 시간이 길어지면 같은 속도에서도 더 멀리 이동한다. 노면이 미끄러우면 마찰계수가 작아지고, 그래프는 더 가파르게 올라간다. 질량은 현실적인 변수이지만, 단순 마찰 모델에서는 운동에너지와 마찰력이 함께 커지기 때문에 정지거리 식에서 약분된다.

결국 이 탐구는 속도를 줄이면 안전하다는 익숙한 말을 수학과 과학으로 설명하는 활동이다. 단순한 경고가 아니라, 왜 그런지를 식과 그래프, 시뮬레이션으로 확인하는 것이다.

**핵심 정리**  
정지거리 문제는 현실의 안전 문제이면서 동시에 이차함수 문제이다. 속도, 반응 시간, 노면 상태가 정지거리에 미치는 영향을 식과 그래프로 설명할 수 있다.
"""
)
